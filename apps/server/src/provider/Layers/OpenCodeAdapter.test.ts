...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapter turn lifecycle", (it) => {
  ...
  it.effect("opens a turn when a finished background subagent wakes the session", () =>
    Effect.gen(function* () {
      ...
      const session = sessions.find((candidate) => candidate.threadId === threadId);
      NodeAssert.equal(session?.status, "ready");
      // The wake-up turn starts at OpenCode's own prompt, so rewinding it cuts there.
      const prompt = runtimeMock.state.promptCalls[0] as { messageID: string };
      NodeAssert.deepEqual(
        (session?.resumeCursor as { turnStartMessageIds?: Array<string> } | undefined)
          ?.turnStartMessageIds,
        [prompt.messageID, "msg-wake"],
      );
  ...
  it.effect("stops live subagents and the old session when the thread rewinds", () =>
    Effect.gen(function* () {
      ...
      // The rewound turn is the one just sent, so its prompt is in history.
