...
describe("DesktopEarlyElectronStartup", () => {
  ...
  it("resolves the early linux Electron switches", () => {
    ...
    assert.deepEqual(options, {
      isDevelopment: true,
      linuxWmClass: "cody-dev",
      linuxDesktopEntryName: "com.vedpandey.cody.dev.desktop",
      passwordStore: "gnome-libsecret",
    });
  });

  it("keeps implicit development state under ~/.cody/dev when T3CODE_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.cody/dev/desktop-settings.json");
