          ...
          FileSystem.layerNoop({
            exists: (path) =>
              input.legacyPathProbeError
                ...
                : Effect.succeed(input.legacyPathExists === true && path.includes("Cody (Alpha)")),
            readFileString: () =>
...
describe("DesktopAppIdentity", () => {
  it.effect("does not reuse T3's existing Electron profile or single-instance lock", () =>
    DesktopAppIdentity.resolveUserDataPath.pipe(
      Effect.tap((path) =>
        Effect.sync(() => {
          assert.equal(path, "/Users/alice/Library/Application Support/cody");
        }),
      ),
      Effect.provide(
        Layer.mergeAll(
          ...
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(path.endsWith("/T3 Code (Alpha)") || path.endsWith("/t3code")),
          }),
        ),
      ),
    ),
  ...
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Cody (Alpha)");
      }),
  ...
  it.effect("preserves failures while inspecting the legacy userData path", () => {
    const legacyPath = "/Users/alice/Library/Application Support/Cody (Alpha)";
