  ...
  it.effect("uses canonical source-tree icons for unpackaged development", () =>
    Effect.gen(function* () {
      ...
      const fileSystemLayer = FileSystem.layerNoop({
        exists: (path) => Effect.succeed(String(path).includes("/assets/cody/")),
      });
      ...
      assert.match(Option.getOrThrow(icons.ico), /assets\/cody\/icon\.ico$/);
      assert.match(Option.getOrThrow(icons.png), /assets\/cody\/icon-1024\.png$/);
